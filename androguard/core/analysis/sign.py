...
from CheckProtect.androguard.core.analysis.analysis import TAINTED_PACKAGE_CREATE, TAINTED_PACKAGE_CALL
                    ...
                    if path.get_class_name() in self.classes_names:
                        ...
                        if present == True:
                            l.append((path.get_idx(), "P%s{%s%s%s}" % (
                                PACKAGE_ACCESS[path.get_access_flag()], path.get_class_name(), path.get_name(),
                                path.get_descriptor()) ))
                        else:
                            l.append((path.get_idx(), "P%s" % (PACKAGE_ACCESS[path.get_access_flag()]) ))
                else:
